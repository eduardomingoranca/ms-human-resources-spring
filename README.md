...
serviço de autorização (hr-oauth) com token JWT. Todos os microservices serão registrados em um servidor Discovery Eureka, e a 
API Gateway para rotiar as requisições aos microservices além de um servidor de configurações dos microservices.

## Autenticação e Autorização
Autenticação é quando se informa as credenciais, já a autorização é para permissão de acesso a algum 
recurso do sistema. 

![security](https://github.com/eduardomingoranca/ms-spring/blob/main/autenticacao.PNG)

O processo do oauth possui três autores basicos que são o servidor de autorização, o servidor de recursos e
o cliente (aplicação) que vai acessar esses servidores. O servidor de autorização recebe as credenciais do usuario 
e da aplicação apos isso devolve para a aplicação o token que é um código que estará assinado, o token terá os claims 
que são as reivindicações onde possui o usuário, os perfis de usuário no sistema, o tempo de expiração do token, ou seja é o 
conteúdo enviado dentro do token quando for tentar acessar algum recurso. O servidor de recurso recebe o token com a URI, e o 
servidor após analisar suas autorizações devolve o recurso.
